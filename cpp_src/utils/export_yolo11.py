...
def yolov8_export(weights, device, inplace=True, fuse=True):
    ...
    for m in model.modules():
        ...
        if hasattr(m, "inplace"):
            m.inplace = inplace
        elif t.__name__ == "Upsample" and not hasattr(
...
def main(args):
    ...
    onnx_name = args.weights.split(".")[0]
    onnx_output_file = f"{onnx_name}.onnx"
...
def parse_args():
    ...
    parser.add_argument(
        "--batch", type=int, default=1, help="Static batch-size"
    )
